...
//#region node_modules/.nitro/vite/services/ssr/assets/census.server-BIhNYANX.js
if (typeof window !== "undefined") throw new Error("@/lib/civic/census.server is server-only.");
var BASE = "https://geocoding.geo.census.gov/geocoder/geographies/onelineaddress";
var TTL_MS = 864e5;
...
var ZIP_ONLY = /^\d{5}(-\d{4})?$/u;
function readDistrict(record) {
	const cdKey = Object.keys(record).find((k) => /^CD\d+$/u.test(k));
	return {
		number: cdKey ? String(record[cdKey]) : void 0,
		stateFips: String(record.STATE ?? ""),
		congress: cdKey ? Number.parseInt(cdKey.slice(2), 10) : NaN
	};
}
/**
...
*/
async function lookupDistrict(query) {
	const address = query.trim();
	if (!address) return {
		ok: false,
		error: {
			kind: "bad_input",
			message: "Enter a street address to resolve a district."
		},
		provenance: provenanceNow("U.S. Census Bureau", "geocoder")
	};
	if (ZIP_ONLY.test(address)) return {
		ok: true,
		stale: false,
		data: {
			kind: "needs_address",
			message: "A ZIP code does not determine a congressional district — ZIPs regularly span two or more. Enter a street address."
		},
		provenance: provenanceNow("U.S. Census Bureau", "geocoder")
	};
	...
	return cached(`census:${address.toLowerCase()}`, TTL_MS, async () => {
		...
		const record = Object.entries(match.geographies ?? {}).find(([name]) => /congressional district/iu.test(name))?.[1]?.[0];
		if (!record) return {
			provenance,
			data: {
				kind: "no_match",
